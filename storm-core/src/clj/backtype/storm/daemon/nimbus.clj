      ...
      (^void submitTopologyWithOpts
        ...
        (try
          ...
          (let [storm-id (str storm-name "-" @(:submitted-count nimbus) "-" (current-time-secs))
                ...
                ;just iterating over total-storm-conf
              ;  (println ("Printing the total-storm-conf"))


                storm-cluster-state (:storm-cluster-state nimbus)]
            (doseq [[k v] (map vector (keys total-storm-conf ) (vals total-storm-conf ))]
              (println (str "Printing the total-storm-conf" k " " v)))
